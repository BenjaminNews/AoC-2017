    ...
    public void setup(String input) {
        for(String action : input.split("\n")) {
            if(action.startsWith("value")) {
                ...
                int chipValue = Integer.parseInt(values[1]);
                int botNumber = Integer.parseInt(values[5]);
                if(bots.containsKey(botNumber)) {
                    Bot bot = bots.get(botNumber);
                    bot.addChip(chipValue);
                    bots.put(botNumber, bot);
                } else {
                    Bot bot = new Bot(botNumber);
                    bot.chipOne = chipValue;
                    bots.put(botNumber, bot);
                }
            } else {
                actions.add(action);
...
//        System.out.println("\n");
    ...
    private class Bot {
        ...
        public String toString() {
            return "bot " + number + ": chip one value: " + chipOne + " chip two value: " + chipTwo;
        }
    }

    private static final String input = "bot 119 gives low to bot 18 and high to bot 3\n" +
