...
public class AdventOfCodeDayTwenty {
    ...
    HashMap<Integer, Particle> allParticles = new HashMap<>();
    ...
    public void setUp(String input) {
        ...
        for(int i = 0; i < particles.length; i++) {
            String[] elements = particles[i].split(", ");

            String positions = elements[0].substring(elements[0].indexOf('<') + 1, elements[0].indexOf(">"));

            double positionOne = Double.parseDouble(positions.split(",")[0].trim());
            double positionTwo = Double.parseDouble(positions.split(",")[1].trim());
            double positionThree = Double.parseDouble(positions.split(",")[2].trim());

            String velocity = elements[1].substring(elements[1].indexOf('<') + 1, elements[1].indexOf(">"));

            double velocityOne = Double.parseDouble(velocity.split(",")[0].trim());
            double velocityTwo = Double.parseDouble(velocity.split(",")[1].trim());
            double velocityThree = Double.parseDouble(velocity.split(",")[2].trim());

            String acceleration = elements[2].substring(elements[2].indexOf('<') + 1, elements[2].indexOf(">"));

            double accelerationOne = Double.parseDouble(acceleration.split(",")[0].trim());
            double accelerationTwo = Double.parseDouble(acceleration.split(",")[1].trim());
            double accelerationThree = Double.parseDouble(acceleration.split(",")[2].trim());

            Particle particle = new Particle(positionOne, positionTwo, positionThree,
                                                                                       velocityOne, velocityTwo, velocityThree,
                                                                                       accelerationOne, accelerationTwo, accelerationThree);
            allParticles.put(particleNumber, particle);
            particleNumber++;
        }
    }
    ...
    public void move(int numberOfMoves) {
        for(int i = 0; i < numberOfMoves; i++) {
            for (int particle : allParticles.keySet()) {
                allParticles.get(particle).move();
            }
        }
    }

    public int getParticleClosestToZero() {
        double distanceFromZero = Integer.MAX_VALUE; //set to a hgih value to work down from
        int particleNumber = 0;

        for (int particle : allParticles.keySet()) {
            double particlesDistanceFromZero = allParticles.get(particle).getDistanceFromStartingPoint();
            if(particlesDistanceFromZero < distanceFromZero) {
                distanceFromZero = particlesDistanceFromZero;
    ...
    public void movePartTwo(int numberOfMoves) {
        for(int i = 0; i < numberOfMoves; i++) {
            for (int particle : allParticles.keySet()) {
                allParticles.get(particle).move();
            }
    ...
    public void collisions() {
        ...
        for (int particleOne : allParticles.keySet()) {
            Particle checkParticleOne = allParticles.get(particleOne);
            String particleOnePosition = checkParticleOne.getPosition();
            for (int particleTwo : allParticles.keySet()) {
                if(particleOne != particleTwo) {
                    Particle checkParticleTwo = allParticles.get(particleTwo);
                    String particleTwoPosition = checkParticleTwo.getPosition();
                    if(particleOnePosition.equals(particleTwoPosition)) {
                        if(!particlesToRemove.contains(particleOne)) {
                            particlesToRemove.add(particleOne);
                        }
                        if(!particlesToRemove.contains(particleTwo)) {
                            particlesToRemove.add(particleTwo);
                        }
                    }
                }
            }
        ...
        for(int particle = 0; particle < particlesToRemove.size(); particle++) {
            allParticles.remove(particlesToRemove.get(particle));
        }
    }

    public int getNumberOfRemainingParticles() {
        return allParticles.size();
    }

    private class Particle {

        private double positionValueOne = 0;
        private double positionValueTwo = 0;
        private double positionValuethree = 0;

        private double velocityValueOne = 0;
        private double velocityValueTwo = 0;
        private double velocityValueThree = 0;

        private double accelerationValueOne = 0;
        private double accelerationValueTwo = 0;
        private double accelerationValueThree = 0;

        private Particle(double positionValueOne, double positionValueTwo, double positionValueThree,
                         double velocityValueOne, double velocityValueTwo, double velocityValueThree,
                         double accelerationValueOne, double accelerationValueTwo, double accelerationValueThree) {

            this.positionValueOne = positionValueOne;
            this.positionValueTwo = positionValueTwo;
            this.positionValuethree = positionValueThree;

            this.velocityValueOne = velocityValueOne;
            this.velocityValueTwo = velocityValueTwo;
            this.velocityValueThree = velocityValueThree;

            this.accelerationValueOne = accelerationValueOne;
            this.accelerationValueTwo = accelerationValueTwo;
            this.accelerationValueThree = accelerationValueThree;
        }

        private void move() {
            velocityValueOne += accelerationValueOne;
            velocityValueTwo += accelerationValueTwo;
            velocityValueThree += accelerationValueThree;

            positionValueOne += velocityValueOne;
            positionValueTwo += velocityValueTwo;
            positionValuethree += velocityValueThree;
        }

        private double getDistanceFromStartingPoint() {
            return Math.abs(positionValueOne) + Math.abs(positionValueTwo) + Math.abs(positionValuethree);
        }

        private String getPosition() {
            return positionValueOne + "-" + positionValueTwo + "-" + positionValuethree;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("particle number closest to zero: " + result);

        adventOfCodeDayTwenty = new AdventOfCodeDayTwenty();

        adventOfCodeDayTwenty.setUp(input);

        adventOfCodeDayTwenty.movePartTwo(100);

        result = adventOfCodeDayTwenty.getNumberOfRemainingParticles();

        System.out.println("remaining particles: " + result);
    }

    private static final String input = "p=<-1724,-1700,5620>, v=<44,-10,-107>, a=<2,6,-9>\n" +
